...
export default function NormasPanel({
  ...
  async function handleUpload(e: React.FormEvent) {
    e.preventDefault();
    if (!file) {
      setError("Seleccione un archivo PDF o TXT");
      return;
    }
    if (!titulo.trim()) {
    ...
    try {
      ...
      await uploadNorma({
        ...
        etiquetas: etiquetas.trim() || undefined,
        proyectoId,
  ...
  return (
    <div className="normas-panel">
      ...
      <div className="normas-busqueda">
        <input
          placeholder="Buscar por título, descripción, etiquetas o contenido"
          value={busqueda}
          onChange={(e) => setBusqueda(e.target.value)}
        />
        <button className="btn" onClick={() => void buscar()}>
          Buscar
        </button>
        <button
          className="btn"
      ...
      <div className="normas-secciones">
        <div className="normas-col">
          ...
          <div className="normas-list">
            {adjuntas.length === 0 ? (
              <div className="empty-state">No hay normas adjuntas</div>
            ) : (
              adjuntas.map((n) => (
                <div key={n.id} className="norma-item">
                  <div className="norma-main">
                    ...
                    {n.descripcion && (
                      <div className="norma-desc">{n.descripcion}</div>
                    )}
                    {n.etiquetas && (
                      <div className="norma-tags">{n.etiquetas}</div>
                    )}
                  </div>
                  <div className="norma-actions">
                    ...
                    <button
                      className="btn small"
                      onClick={async () => {
                        await detachNorma(n.id, { proyectoId });
                        await loadAdjuntas();
                      }}
                    >
                      Quitar
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (!confirm("Eliminar norma?")) return;
                        await deleteNorma(n.id);
                        await loadAdjuntas();
                        await buscar();
                      }}
                    >
                ...
                <div key={n.id} className="norma-item">
                  ...
                  <div className="norma-actions">
                    ...
                    <button
                      className="btn small"
                      onClick={async () => {
                        await attachNorma(n.id, { proyectoId });
                        await loadAdjuntas();
                      }}
                    >
                      Añadir al proyecto
                    ...
                    <div className="attach-tarea">
                      <select
                        ...
                        onChange={async (e) => {
                          ...
                          if (val) {
                            await attachNorma(n.id, { tareaId: val });
                            alert("Asociada a tarea");
                          }
