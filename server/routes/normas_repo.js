...
const excelStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    const now = new Date();
    const dir = path.join(getUploadsBase(), 'normas_repo', 'imports', String(now.getFullYear()), String(now.getMonth() + 1).padStart(2, '0'));
...
function excelFilter(req, file, cb) {
  ...
  ].includes(file.mimetype) || file.originalname.endsWith('.xlsx') || file.originalname.endsWith('.csv');
  if (!ok) return cb(new Error('Formato no soportado (esperado .xlsx o .csv)'));
  cb(null, true);
}
const excelUpload = multer({ storage: excelStorage, limits: { fileSize: 50 * 1024 * 1024 }, fileFilter: excelFilter });

// Storage for evidencias (images)
const imageStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    const now = new Date();
    const dir = path.join(getUploadsBase(), 'evidencias_normas', String(now.getFullYear()), String(now.getMonth() + 1).padStart(2, '0'));
    fs.mkdirSync(dir, { recursive: true });
    cb(null, dir);
  },
  filename: function (req, file, cb) {
...
const imageUpload = multer({ storage: imageStorage, limits: { fileSize: 10 * 1024 * 1024 }, fileFilter: imageFilter });
...
async function upsertNormaRepo(row) {
  const fields = ['codigo','titulo','descripcion','categoria','subcategoria','incumplimiento','severidad','etiquetas','fuente'];
  // no upsert by codigo if missing
  if (row.codigo) {
    const [rows] = await pool.query('SELECT id FROM normas_repo WHERE codigo = ?', [row.codigo]);
    if (rows && rows.length > 0) {
      ...
      await pool.query(
        ...
        [row.titulo || null, row.descripcion || null, row.categoria || null, row.subcategoria || null, row.incumplimiento || null, row.severidad || null, row.etiquetas || null, row.fuente || null, id]
      );
  ...
  if (!row.codigo && row.titulo) {
    const keyParams = [row.titulo?.trim() || null, row.categoria?.trim() || null, row.subcategoria?.trim() || null, row.fuente?.trim() || null];
    ...
    if (rows2 && rows2.length > 0) {
      ...
      await pool.query(
        ...
        [row.descripcion || null, row.incumplimiento || null, row.severidad || null, row.etiquetas || null, id]
      );
      return { id, updated: true };
    }
  }
  const [result] = await pool.query(
    ...
    [row.codigo || null, row.titulo, row.descripcion || null, row.categoria || null, row.subcategoria || null, row.incumplimiento || null, row.severidad || null, row.etiquetas || null, row.fuente || null]
  );
...
router.post(
  ...
  (req, res, next) => {
    excelUpload.single('file')(req, res, (err) => {
      if (err) {
        ...
        console.error('[normas-repo/import] multer error:', code || '', err && err.message ? err.message : err);
        let friendly = 'Error procesando archivo';
        if (code === 'LIMIT_FILE_SIZE') friendly = 'Archivo demasiado grande (límite 50MB)';
        return res.status(400).json({ error: friendly, detail: err && err.message ? err.message : String(err), code });
      }
      if (!req.file) {
        console.error('[normas-repo/import] no file received. headers=', req.headers);
        return res.status(400).json({ error: 'Archivo no recibido', detail: 'No se recibió el archivo en el campo "file"' });
      }
      next();
    });
  },
  async (req, res) => {
    try {
      if (!XLSX) return res.status(500).json({ error: 'Dependencia XLSX no instalada' });
      const file = req.file;
      if (!file) return res.status(400).json({ error: 'Archivo requerido' });
  console.log('[normas-repo/import] received file:', { originalname: file.originalname, mimetype: file.mimetype, size: file.size, path: file.path });
  const wb = XLSX.readFile(file.path);
      const sheetName = wb.SheetNames[0];
      const sheet = wb.Sheets[sheetName];
  const rowsAoA = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '' });
      if (!Array.isArray(rowsAoA) || rowsAoA.length === 0) return res.status(400).json({ error: 'Hoja vacía' });

      // Intento 1: detectar fila de encabezados
      const headerTokens = ['codigo','código','titulo','título','descripcion','descripción','categoria','categoría','subcategoria','subcategoría','incumplimiento','hallazgo','severidad','nivel','etiquetas','tags','fuente','origen','articulo','artículo'];
      let headerIdx = -1;
      for (let i = 0; i < Math.min(10, rowsAoA.length); i++) {
        const row = rowsAoA[i] || [];
        const norm = row.map((c) => normalizeHeader(c));
        if (norm.some((h) => headerTokens.includes(h))) { headerIdx = i; break; }
      }

  let created = 0, updated = 0, errors = 0, totalRows = 0;
      ...
      if (headerIdx >= 0) {
        ...
        for (const r of dataRows) {
          ...
          const record = {};
          headers.forEach((h, i) => { if (h) record[h] = r[i]; });
          // Construir título de forma tolerante
          const categoria = (record.categoria || record.seccion || record.sección || '').toString().trim() || null;
          const descripcion = (record.descripcion || record.descripcion_ || record.incumplimiento || record.titulo || record.nombre || record.norma || '').toString().trim();
          const articulo = (record.articulo || record.artículo || record.fuente || record.origen || '').toString().trim() || null;
          record.titulo = descripcion;
          if (!record.titulo) {
            // Si no hay campos útiles, saltar sin contar como error
            const meaningful = [record.descripcion, record.incumplimiento, record.categoria, record.subcategoria, record.fuente, record.articulo, record['incumplimientos_electricos']].filter(v => String(v||'').trim() !== '');
            if (meaningful.length === 0) {
              // Como último recurso, tomar primeras 1-2 celdas no vacías de la fila original
              const nonEmptyCells = (r || []).map(c => String(c||'').trim()).filter(Boolean);
              if (nonEmptyCells.length === 0) continue;
              record.titulo = nonEmptyCells.slice(0,2).join(' - ').slice(0, 160);
            } else {
          ...
          try {
            ...
            if (resp.created) created++; else if (resp.updated) updated++; else created++;
          } catch (e) {
            console.warn('Upsert norma_repo (header mode) error:', e && e.message ? e.message : e);
            errors++;
          }
        }
      } else {
        ...
        for (const row of rowsAoA) {
          ...
          if (nonEmpty.length === 1) {
            ...
            if (nh === 'incumplimientos_electricos' || nh === 'articulo') continue;
          ...
          const col2 = nonEmpty.length >= 3 ? nonEmpty[nonEmpty.length - 1] : '';// articulo
          ...
          const isHeaderRow = [n0,n1,n2].includes('incumplimientos_electricos') || [n0,n1,n2].includes('articulo');
          ...
          try {
            ...
            console.warn('Upsert norma_repo (fallback mode) error:', e && e.message ? e.message : e, 'record=', record);
            errors++;
          }
          totalRows++;
        }
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const params = [];
      if (search) {
        where.push('(titulo LIKE ? OR descripcion LIKE ? OR incumplimiento LIKE ? OR etiquetas LIKE ? OR codigo LIKE ?)');
        for (let i = 0; i < 5; i++) params.push(`%${search}%`);
      }
      if (categoria) { where.push('categoria LIKE ?'); params.push(`%${categoria}%`); }
...
router.post(
  ...
  async (req, res) => {
    try {
      const { codigo, titulo, descripcion, categoria, subcategoria, incumplimiento, severidad, etiquetas, fuente } = req.body || {};
      const [result] = await pool.query(
        'INSERT INTO normas_repo (codigo, titulo, descripcion, categoria, subcategoria, incumplimiento, severidad, etiquetas, fuente) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [codigo || null, titulo, descripcion || null, categoria || null, subcategoria || null, incumplimiento || null, severidad || null, etiquetas || null, fuente || null]
      );
      const [rows] = await pool.query('SELECT * FROM normas_repo WHERE id = ?', [result.insertId]);
  ...
  async (req, res) => {
    try {
      ...
      await pool.query(
        ...
        [codigo || null, titulo || null, descripcion || null, categoria || null, subcategoria || null, incumplimiento || null, severidad || null, etiquetas || null, fuente || null, id]
      );
      const [rows] = await pool.query('SELECT * FROM normas_repo WHERE id = ?', [id]);
...
router.post(
  ...
  (req, res, next) => {
    imageUpload.single('file')(req, res, (err) => {
      if (err) return res.status(400).json({ error: 'Error procesando imagen', detail: err && err.message ? err.message : String(err) });
      next();
    });
  },
  checkValidation,
  async (req, res) => {
    try {
      const { id } = req.params;
      const file = req.file;
      const comentario = req.body && req.body.comentario ? String(req.body.comentario) : null;
      if (!file) return res.status(400).json({ error: 'Imagen requerida' });
      // verificar que norma exista
      const [rows] = await pool.query('SELECT id FROM normas_repo WHERE id = ?', [id]);
      ...
      if (sharp) {
        try {
          ...
          await sharp(outPath).resize(480).jpeg({ quality: 70 }).toFile(thumbOut);
          // remove original if different
          if (outPath !== file.path) { try { fs.unlinkSync(file.path); } catch {}
          }
          finalPath = outPath;
          thumbPath = thumbOut;
        } catch (e) {
          console.warn('sharp failed, using original image', e && e.message ? e.message : e);
        }
      }

      const mime = 'image/jpeg';
      const size = fs.existsSync(finalPath) ? fs.statSync(finalPath).size : file.size;
      const [result] = await pool.query(
...
router.get(
  ...
  async (req, res) => {
    try {
      const { id } = req.params;
      const [rows] = await pool.query('SELECT * FROM normas_repo_evidencias WHERE norma_repo_id = ? ORDER BY created_at DESC', [id]);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const [rows] = await pool.query('SELECT image_path, thumb_path FROM normas_repo_evidencias WHERE id = ?', [evidenciaId]);
      if (!rows || rows.length === 0) return res.status(404).json({ error: 'Evidencia no encontrada' });
      const img = rows[0];
      await pool.query('DELETE FROM normas_repo_evidencias WHERE id = ?', [evidenciaId]);
      try { if (img.image_path && fs.existsSync(img.image_path)) fs.unlinkSync(img.image_path); } catch {}
      try { if (img.thumb_path && fs.existsSync(img.thumb_path)) fs.unlinkSync(img.thumb_path); } catch {}
      res.status(204).send();
    } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      if (req.query.ids) {
        ...
        const categoria = req.query.categoria ? String(req.query.categoria) : '';
        const severidad = req.query.severidad ? String(req.query.severidad) : '';
        if (search) { where.push('(titulo LIKE ? OR descripcion LIKE ? OR incumplimiento LIKE ? OR etiquetas LIKE ? OR codigo LIKE ?)'); for (let i=0;i<5;i++) params.push(`%${search}%`); }
        if (categoria) { where.push('categoria LIKE ?'); params.push(`%${categoria}%`); }
        if (severidad) { where.push('severidad LIKE ?'); params.push(`%${severidad}%`); }
      }
      const whereSql = where.length ? ('WHERE ' + where.join(' AND ')) : '';
  ...
  const colW = [180, 300, 100];
      ...
      const drawRow = (cells, styles = {}) => {
        ...
        const heights = cells.map((text, i) => {
          const h = doc.heightOfString(String(text || ''), { width: colW[i] - 6 });
      ...
      // Data rows
      rows.forEach(r => {
        drawRow([r.categoria || '', r.descripcion || r.titulo || '', r.fuente || '']);
      });
